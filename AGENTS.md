...
## React DOM ownership and overlay safety

- **Do not manually move, replace, or remove DOM nodes rendered by React.** Never call `removeChild`, `appendChild`, `insertBefore`, `replaceChild`, or `.remove()` on a React-owned element. This has repeatedly caused `Cannot read properties of null (reading 'removeChild')` under React 19, concurrent rendering, and hot updates.
- **Avoid transient `createPortal(..., document.body)` for dialogs, selection rectangles, menus, and sheets.** Prefer a stably mounted element whose visibility is controlled by state, or the native top-layer `<dialog>` API. New Agent workspace overlays must not introduce another body Portal.
- If a Portal is unavoidable, use a dedicated persistent host owned by React, keep both host and portal subtree mounted for the lifetime of the provider, and never let third-party or imperative code mutate that subtree.
- Imperatively created temporary nodes are allowed only when React never rendered or owns them, such as a clipboard fallback `<textarea>` created and removed entirely inside one function.
- When changing overlay or selection UI, verify repeated open/close, rapid state changes, route changes, and development hot updates. Treat any `removeChild` runtime error as a DOM ownership bug, not as a harmless refresh issue.
...
- `DATABASE_URL` — `sqlite:///./zhicui.db` (dev) or `postgresql://zhicui:Zhc2026abc@localhost:5432/zhicui` (prod)
