...
import { AlertCircle, Calendar, CheckSquare, TrendingUp, Layers, Trash2 } from 'lucide-react';
...
export default function PlanCard({ plan, onDelete }: PlanCardProps) {
  ...
  return (
    <article
      className={`glass-card plan-card group relative overflow-hidden ${
        isComplete ? 'border-accent-emerald/40 ring-1 ring-accent-emerald/20' : ''
      }`}
    >
      <Link
        href={`/plans?id=${plan.id}`}
        className="plan-card__link text-foreground no-underline"
        aria-label={`打开计划：${plan.title}`}
      >
        <div className="flex items-start justify-between gap-2 mb-3 relative pr-9">
        ...
        <div className="flex items-center justify-between text-xs text-foreground-muted">
          ...
          <span className="text-accent-emerald opacity-60 group-hover:opacity-100 transition-opacity">
            查看详情 →
          </span>
        </div>
      </Link>
