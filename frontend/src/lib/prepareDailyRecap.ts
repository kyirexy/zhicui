...
export async function prepareDailyRecap(
  ...
  check();
  // 准备阶段固定原日期，跨午夜也继续处理这批资料；首页读取今日列表才使用专用接口。
  const fetchRecap = (date?: string) => getDailyRecap(recap.timezone, signal, date);
  ...
  try {
    ...
    save(key, state);
    check();
    const content = `请根据本会话选中的 ${noteIds.length} 条视频资料，整理 ${recap.date} 的「${periodLabel}」。\n`
